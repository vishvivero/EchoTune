...
struct PurchaseView: View {
    ...
    private var primaryCTA: String {
        licenseManager.isTrialExpired ? "Purchase Now" : "Unlock EchoTune Pro"
    }
    ...
    private var featuresSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                ...
                FeatureRow(icon: "lock.shield", color: .green, text: "Private by default — local models keep audio on your Mac")
                FeatureRow(icon: "keyboard", color: .orange, text: "Global keyboard shortcut (⌥ Option key)")
    ...
    private var restoreSection: some View {
        VStack(spacing: 12) {
            Text("Have a license key from a previous purchase?")
                .font(.caption)
                .foregroundColor(.secondary)
