...
class ShortcutManager: NSObject {
    ...
    // Default shortcut: Option key (single press)
    private var shortcutKeyCode: UInt32 = 58 // Option key alone
    private var shortcutModifiers: UInt32 = UInt32(NSEvent.ModifierFlags.option.rawValue)
    ...
    // Reset shortcut to default (Option key single press)
    func resetToDefault() {
        shortcutKeyCode = 58
        shortcutModifiers = UInt32(NSEvent.ModifierFlags.option.rawValue)
