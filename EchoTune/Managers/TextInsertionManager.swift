    ...
    func insertText(_ text: String, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        if let appName = getFrontmostApplication() {
            ...
            if isBrowserApp(appName) {
                debugLog("🌐 Browser detected! Trying direct paste into the active web text area first")

                if insertViaPaste(processedText) {
                    debugLog("✅ Browser paste completed successfully")
                    debugLog("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                    completion(.success(()))
                    return
                }

                debugLog("⚠️ Browser paste fallback: switching to character-by-character typing")
