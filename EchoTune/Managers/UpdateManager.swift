...
class UpdateManager: ObservableObject {
    ...
    private func showUpdateAvailableAlert(info: UpdateInfo) {
        ...
        if alert.runModal() == .alertFirstButtonReturn {
            // Deliberately open the official download page rather than the
            // feed-supplied file URL: the manual path has no signature
            // verification, so feed content must never pick what gets opened.
            if let url = URL(string: "https://echotune.app/download") {
